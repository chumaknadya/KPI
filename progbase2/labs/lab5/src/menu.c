...
void  convert_string_xml(char *str) {
 char  * xml = ".xml";
 if(strstr(str,xml) == NULL) {
     strcat(str,xml);
 }

}
void  convert_string_json(char  * str) {
  char  * json = ".json";
  if (strstr(str,json) == NULL) {
     strcat(str,json);
  }
}
void  convert_string_txt(char  *str) {
  char  * txt = ".txt";
  if ( strstr(str,txt) == NULL) {
       strcat(str,txt); 
  }
}
void print_format_input(void){
  printf("1.Зчитати з формату txt\n");
  printf("2.Зчитати з формату xml\n");
  printf("3.Зчитати з формату json\n");
  printf("Настисніть 0 ,щоб вийти\n");
}
List * choose_format_for_using(List * list) {
      ...
      int box_width = 100;
       ...
       while (keyDown_6 != '0') {
             ...
             keyDown_6 = conGetChar();
                switch(keyDown_6) {
                         case '1': {
                           printf("Введіть назву файлу для зчитування\n");
                           fgets(str,16,stdin);
                           str[strlen(str) -1] = '\0';
                           convert_string_txt(str);
                           list = Storage_load(str);
                         ...
                         case '2': {
                           ...
                           fgets(str,16,stdin);
                           str[strlen(str) -1] = '\0';
                           convert_string_xml(str);
                            list = Storage_readAsXml(str);
                           clearLine(' ',11,box_width-1,20,BG_RED);
                           conMove(12,1);
                           break;
                         }
                         case '3' : {
                           ...
                           fgets(str,16,stdin);
                           str[strlen(str) -1] = '\0';
                           convert_string_json(str);
                            list = Storage_readAsJson(str);
                           clearLine(' ',11,box_width-1,20,BG_RED);
                           conMove(12,1);
                           break;
                         }
...
void print_formats(void) {
  printf("1.Записати у формат txt\n");
  printf("2. Записати у формат json\n");
  printf("3.Записати у формат xml \n");
  printf("Настисніть 0 ,щоб перейти в головне меню\n");
}
...
int menu (void) {
    ...
    Computer_Game * input_games = NULL;
    List * computer_games = List_new();
          ...
          switch (keyDown_1) {
              case '1': {
               ...
               case '2': {
                 clearLine(' ',6,box_width-1,40,BG_RED);
                 conMove(6,1);
                 computer_games = choose_format_for_using(computer_games);
                 clearLine(' ',6,box_width-1,40,BG_RED);
                 ...
                 while (keyDown_3 != '0') {
                    ...
                    switch(keyDown_3) {
                          case '1': {
                            ...
                            input_games = Computer_Game_new(" ", " ", 0,0.0," "," ");
                            Set_Computer_Game_data(input_games);
                            List_insert(computer_games, position, input_games);
                          ...
                          case '3': {
                            printf("Введіть позицію list,для якого ви хочете зробити зміни \n");
                            int position = input_position_of_element(position,computer_games);
                            input_games = Computer_Game_Set_field (List_get(computer_games, position));
                            clearLine(' ',11,box_width-1,20,BG_RED);
                            conMove(12,1);
                            print_list(computer_games);
                    ...
                    while (keyDown_5 != '0') {
                       ...
                       switch(keyDown_5) {
                         ...
                         case '2': {
                           ...
                           str[strlen(str) -1] = '\0';
                           convert_string_json(str);
                           Storage_writeAsJson(str, computer_games);
                           clearLine(' ',11,box_width-1,20,BG_RED);
                           conMove(12,1);
                           break;
                         }
                         case '3' : {
                           ...
                           str[strlen(str) -1] = '\0';
                           convert_string_xml(str);
                           Storage_writeAsXml(str, computer_games);
                           clearLine(' ',11,box_width-1,20,BG_RED);
                           conMove(12,1);
                           break;
                         }
