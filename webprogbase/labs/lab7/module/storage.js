...
searchByBreed = async function (x_breed){
    ...
    return animalsByBreed.length > 0 ? animalsByBreed : null;
}
create = async function (x_breed,x_description,x_picture,x_facts,x_picture_type) {
    let sql = format("INSERT INTO maininformation(breed,description,facts,picture,picture_type) VALUES ($1,$2,$3,$4,$5)");
    //console.log(sql)
    let newAnimal = await db.query(sql,[x_breed,x_description,x_facts,x_picture,x_picture_type]);
    return true;
}  
