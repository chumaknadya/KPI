<template >
  <div >
    <page-header @newSearchreq="load" ></page-header>
    <div class="container">
        <div class="row">
            <div class="col-md-4">
                <h3 class="forCreate">CREATE ANIMAL</h3> 
                <create-animal @newAnimalAdd="addNewAnimal"></create-animal> 
            </div>
            <div class="col-md-6">
                <h2>LIST OF ANIMALS</h2>
                <!-- <pagination @changePage="goToPage" :animalsPages="pages"> </pagination> -->
                <div class="carousel-reviews broun-block">
                    <div class="container">
                        <div class="row">
                            <div id="carousel-reviews" class="carousel slide" data-ride="carousel">  
                                <div v-if="animals.length != 0" >  
                                   <div class="item" v-for="(animal,i) in animals" :key="i">
                                       <div class="col-md-4 col-sm-6">
                                            <div class="block-text rel zmin">
                                                <a title="">{{animal.breed}}
                                                <button  id="remove"  v-on:click="openModalWindow(animal)" class="rem">
                                                      <span class="glyphicon glyphicon-remove"></span>
                                                </button>
                                                </a>
                
                                               
                                                <div class="mark"> <img  alt=""  class="animal" 
                                                :src="animal.picture_url"></div>                                            
                                                <p>{{animal.description}}</p>
                                                <ins class="ab zmin sprite sprite-i-triangle block"></ins>
                                            </div>
                                            <div class="person-text rel">  
                                                <a title="" href="/animals/id">{{animal.breed}}</a>
                                                <!--<a title="">breed</a>-->
                                            </div> 
                                        </div> 
                                    </div>
                                    <div class="cl"><pagination @changePage="goToPage" :animalsPages="pages"> </pagination></div>
                                 </div>
                                 <div v-else> 
                                     <h2>List is Empty</h2>
                                 </div>
                          ...
                          <remove-window :animal="animalToRemove" @remove="removeAnimal" @close="closeModalWindow"></remove-window>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    import RemoveWindow from './RemoveWindow.vue'
    import Pagination from './Pagination.vue'
  
export default {
    name: 'AnimalsList',
    components: {PageHeader,RemoveWindow, CreateAnimal,Pagination},
    data () {
        return {
          animals: [],
          search: ' ',
    ...
    methods: {
        load: async function (value) {
                let pages = await AnimalApi.find(value)
                this.pages = Math.ceil(pages.data.length/3)
                console.log(this.pages)
                this.search = value;
                try {
                    let res = await AnimalApi.find(value)
                    //this.animals = res.data
                    this.animals = res.data.slice(0,3)
                } catch(e){console.log(e)}
            
        },
        openModalWindow(animal){
            this.animalToRemove = animal;
        },
        closeModalWindow() {
            this.animalToRemove = null;
        },
        removeAnimal : async function(animal){
            this.closeModalWindow();
            console.log("removingById " + animal.id)
            try {
                await AnimalApi.removeAnimal(animal.id)
                let pages = await AnimalApi.getPages() 
                this.pages = Math.ceil(pages.data/3)
                let res = await AnimalApi.find(this.search)
                //this.animals = res.data
                this.animals = res.data.slice(0,3)
            }catch(e){console.log(e)}
        },
        addNewAnimal: async function(value) {
            console.log( "addNewAnimal "+ value)
            try {
                let res = await AnimalApi.find(this.search)
                //this.animals = res.data
                this.animals = res.data.slice(0,3)
        ...
        goToPage: async function(value){
            ...
            if(this.search != null && this.search != ''){
                ...
                let res = await AnimalApi.find(this.search)
                this.pages = Math.ceil(res.data.length/3)
                console.log("pages in search "+this.pages)
                console.log(res.data.slice((value-1)*3, value*3))
                let paginatedAnimals = res.data.slice((value-1)*3, value*3)
                this.animals = paginatedAnimals
            }else {
                console.log("out of search")
                let res = await AnimalApi.find(this.search)
                console.log("pages out search "+this.pages)
                console.log(res.data.slice((value-1)*3, value*3))
                let paginatedAnimals = res.data.slice((value-1)*3, value*3)
                this.animals = paginatedAnimals
            }
...
<style scoped>
    ...
    .carousel-control.left,.carousel-control.right  {background:none;width:24px;}
